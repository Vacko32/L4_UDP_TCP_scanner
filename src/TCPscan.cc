...
tcp_socket::tcp_socket(char *domain, int port, std::vector<std::string> interfaces_addresses,
  ...
  // we will iterate through them and if they are not in our adress vector we will add them, the
  // source of this code is here:https://beej.us/guide/bgnet/html/#socket , in section 5.1
  struct addrinfo *r;
...
int tcp_socket::craft_packet(std::string source, std::string destination, int port) {
  ...
  if (type1 != type2) {
    ...
    while (std::chrono::steady_clock::now() - start
           < std::chrono::milliseconds(GLOBAL_TIMEOUT / timeout_split_2)) {
      ssize_t recv_got
          = recvfrom(sockfd, resp.data(), resp.size(), 0, (struct sockaddr *)&sender, &sender_len);
      if (recv_got > 0) {
