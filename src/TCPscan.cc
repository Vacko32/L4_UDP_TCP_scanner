...
{
  // This remove the interface from the adrress, i dont know if its an enviroment issue, but setupinterfaces(), does return ipv6 adresses with interface
  ...
  // we will iterate through them and if they are not in our adress vector we will add them, the source of this code is here:https://beej.us/guide/bgnet/html/#socket , 
  // in section 5.1
  struct addrinfo *r;
...
int tcp_socket::craft_packet(std::string source, std::string destination, int port)
{
  ...
  else if (type1 == PF_INET)
  {
    ...
    while (std::chrono::steady_clock::now() - start < std::chrono::milliseconds(GLOBAL_TIMEOUT / timeout_split_2))
    {
      ...
      if (std::chrono::steady_clock::now() - start > std::chrono::milliseconds(GLOBAL_TIMEOUT / 2))
      {
        break;
      }
    }
  ...
  else if (type1 == PF_INET6)
  {
    ...
    memcpy(pseudopacket.get() + sizeof(struct ipv6_pseudo_header), &tcp_header, sizeof(struct tcphdr));

    tcp_header.check = checksum((u_int16_t *)pseudopacket.get(), packet_size);

    int p_size = sizeof(ip6_header) + sizeof(struct tcphdr);
    std::unique_ptr<char[]> packet(new char[p_size]);
    memset(packet.get(), 0, p_size);
    memcpy(packet.get(), &ip6_header, sizeof(ip6_header));
    memcpy(packet.get() + sizeof(ip6_header), &tcp_header, sizeof(struct tcphdr));
    ...
    while (std::chrono::steady_clock::now() - start < std::chrono::milliseconds(GLOBAL_TIMEOUT / timeout_split_2))
    {
      ssize_t recv_got = recvfrom(sockfd, resp.data(), resp.size(), MSG_DONTWAIT, (struct sockaddr *)&sender, &sender_len);
      if (recv_got < 0)
      {
        continue;
      }
      ...
      {
        struct tcphdr *tcp = (struct tcphdr *)(resp.data() + sizeof(struct ip6_hdr));
        if (ntohs(tcp->source) != port)
        {
          continue;
        }
