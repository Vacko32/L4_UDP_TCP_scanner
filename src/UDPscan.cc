...
udp_socket::udp_socket(char* domain, int port, int socktype,
  ...
  if (socktype == SOCK_DGRAM) {
    for (std::string i : adresses) {
      for (std::string j : interfaces_addresses) {  // somehow pass
        setsocket(i, socktype, j);                  // hopefully correct
        int resp = handle_udp_socket(j, i, mainInterface, port);
        if (resp == 3) {
          std::cout << "Port is closed" << "port: " << port << std::endl;
          close(sockfd);
          return;
        }
        close(sockfd);
    ...
    std::cout << "Port is open" << "port: " << port << std::endl;
    return;
  }
}
...
void udp_socket::setsocket(std::string ip_adress, int sock_type, std::string bind_adr) {
  ...
  std::cout << "udp_socket created on adrress" << bind_adr << std::endl;
};
...
int udp_socket::handle_udp_socket(const std::string& source, const std::string& destination,
  ...
  if (x == 1) {
    ...
    if (pcap_set_timeout(handle, GLOBAL_TIMEOUT) != 0) {
    ...
    std::thread t1([this, handle, &found_icpm]() {
      int ret = this->sniff_session_activate(handle);
 ...
 * @return int, 3 if success, else other value
 */
int udp_socket::sniff_session_activate(pcap_t* handle) {
  ...
  while (std::chrono::steady_clock::now() - start
         < std::chrono::milliseconds(GLOBAL_TIMEOUT)) {  // scaning for 3 seconds
    res = pcap_next_ex(handle, &header, &packet);
    if (res == 1) {  // A packet was captured
      std::cout << "Packet captured" << std::endl;
      // we skip first 14 bytes
      struct ip* iph = reinterpret_cast<struct ip*>(const_cast<u_char*>(packet) + 20);
      std::cout << "IP Header Length: " << static_cast<int>(iph->ip_hl * 4) << " bytes"
                << std::flush;
      std::cout << "Protocol: " << static_cast<int>(iph->ip_p) << std::flush;
      if (iph->ip_p == IPPROTO_ICMP) {
        int ipHeaderLength = iph->ip_hl * 4;
        struct icmp* icmph
            = reinterpret_cast<struct icmp*>(reinterpret_cast<u_char*>(iph) + ipHeaderLength);
